...
function renderFeatured(items){
  ...
  if (!featured) return;
  const picks = items.filter(i => ['イベント','カフェ','ドッグラン'].includes(i.category)).slice(0,4);
  featured.innerHTML = `
    <h2 class="subhead">迷った時のおすすめカテゴリ</h2>
    <div class="quick-grid">
      ${picks.map(i=>`<article class="card compact">${cardImage(i)}<p class="badge">${esc(i.category)}</p><h4>${esc(i.title)}</h4><p class="sub">${esc(i.prefecture)}・${esc(i.area)}</p></article>`).join('')}
    </div>
  `;
}
...
function renderApp(items){
  ...
  function render(){
    ...
    const filtered=publicItems.filter(i =>
      ...
      const fresh = String(b.updatedAt||'').localeCompare(String(a.updatedAt||''));
    ...
    document.getElementById('list').innerHTML = filtered.map(i=>`
      <article class="card">
        ${cardImage(i)}
        <div class="badges"><span class="badge">${esc(i.status||'要確認')}</span><span class="badge">${esc(i.category)}</span></div>
        <h3>${esc(i.title)}</h3>
...
setInterval(()=>{ loadItems().then(renderApp).catch(()=>{}); }, 5*60*1000);
